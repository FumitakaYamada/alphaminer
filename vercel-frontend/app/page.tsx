...
export default function Home() {
	...
	const [selectedAlphaChart, setSelectedAlphaChart] = useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				const [alphaResponse, sourceResponse, cumulativeResponse] =
					await Promise.all([
				...
				const cumulative = cumulativeResponse
					? await cumulativeResponse.json()
					: null;
			...
			<div className="container mx-auto px-4 py-8">
				<header className="mb-8">
					...
					<p className="text-gray-600">
						Advanced algorithmic trading strategies performance analysis
					</p>
				</header>
				...
				<Tabs defaultValue="overview" className="space-y-6">
							...
							<Card>
								...
								<CardContent>
									<div className="text-2xl font-bold">
										{(
											alphaData.reduce((sum, a) => sum + a.sharpe_ratio, 0) /
											alphaData.length
										).toFixed(2)}
									</div>
								</CardContent>
					...
					<TabsContent value="leaderboard" className="space-y-4">
						<div
							className="grid"
							style={{ gridTemplateColumns: "42.5% 1fr", gap: "60px" }}
						>
							...
							<div className="flex-none">
								...
								selectedAlpha &&
								nameToClassMap[selectedAlpha] &&
								sourceCode[nameToClassMap[selectedAlpha]] ? (
											...
											<div className="flex justify-between items-center">
												<div>
													<h3 className="text-lg font-semibold">
														{selectedAlphaChart?.replace(/_/g, " ")}
													</h3>
											...
											{(() => {
												const getStrategyData = () => {
													...
													const strategyKey = Object.keys(
														cumulativeReturns.strategies,
													).find(
														(key) =>
															cumulativeReturns.strategies[key].name ===
															selectedAlphaChart,
													);
													...
													const strategy =
														cumulativeReturns.strategies[strategyKey];
												...
												return chartData ? (
													...
													<div className="text-center py-8 text-gray-500">
														<p>
															Cumulative returns data not available for this
															strategy
														</p>
													</div>
												);
									...
									<div className="bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 h-96 flex items-center justify-center">
										<div className="text-center text-gray-500">
											...
											<div className="text-sm">
												Click "View Detail" or "View Chart" on any strategy to
												see details here
											</div>
										</div>
									</div>
						...
						sourceCode[nameToClassMap[selectedAlpha]] ? (
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Select a strategy from the leaderboard to view its source
										code
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="text-center py-8 text-gray-500">
										No strategy selected. Go to the Leaderboard tab and click
										"View Detail" on any strategy.
									</div>
								</CardContent>
							</Card>
						)}
						...
						{selectedAlphaChart ? (
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Select a strategy from the leaderboard to view its
