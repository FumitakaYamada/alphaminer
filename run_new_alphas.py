    ...
    for i, alpha in enumerate(alphas):
        ...
        try:
            ...
            new_results[alpha.name] = {
                ...
                "dates": [
                    d.strftime("%Y-%m-%d")
                    for d in engine.results[alpha.name]["returns"].index
                ],
            }

            print(
                f"  - Sharpe: {metrics['sharpe_ratio']:.3f}, Annual Return: {metrics['annualized_return']:.3f}"
            )
...
def create_leaderboard(results):
    ...
    print("=" * 80)
    print(
        f"{'Rank':<4} {'Alpha Name':<35} {'Sharpe':<8} {'Ann.Ret':<8} {'Calmar':<8} {'MaxDD':<8}"
    )
    print("-" * 80)

    for i, alpha in enumerate(leaderboard_data[:15]):
        print(
            f"{i + 1:<4} {alpha['name'][:34]:<35} "
    ...
    new_alpha_names = [
        ...
        "volume_price_20d",
        "seasonal_trend_252d",
        "return_density_30d_0.005",
        "momentum_decay_5d_20d",
        "vol_breakout_20d_1.5x",
        "mean_reversion_speed_15d",
        "tail_risk_60d_0.05",
        "return_stability_30d",
        "simple_momentum_7d",
        "noise_ratio_20d",
        "simple_ma_20d",
        "vol_rank_15d_45d",
    ...
    new_alphas = [
        alpha for alpha in leaderboard_data if alpha["name"] in new_alpha_names
    ]
    ...
    for i, alpha in enumerate(new_alphas):
        rank = next(
            (
                j + 1
                for j, a in enumerate(leaderboard_data)
                if a["name"] == alpha["name"]
            ),
            0,
        )
        print(
            f"#{rank:<3} {alpha['name']:<35} Sharpe: {alpha['sharpe_ratio']:<7.3f} Return: {alpha['annualized_return']:<7.3f}"
        )
