...
export function PerformanceChart({
	...
	const chartData = topPerformers.map((alpha, index) => ({
		name:
			topPerformers.length <= 30
				? alpha.name.replace(/_/g, " ")
				: alpha.name.replace(/_/g, " ").substring(0, 15) + "...",
		fullName: alpha.name,
		rank: index + 1,
			...
			<Tabs defaultValue="overview" className="space-y-4">
									...
									>
										...
										<Scatter dataKey="return">
											{riskReturnData.map((entry, index) => (
												<Cell
													key={`cell-${index}`}
													fill={entry.isTop10 ? "#3b82f6" : "#d1d5db"}
												/>
											))}
										</Scatter>
									</ScatterChart>
									...
									>
										...
										<Scatter dataKey="return">
											{riskReturnData.map((entry, index) => (
												<Cell
													key={`cell-${index}`}
													fill={entry.isTop10 ? "#dc2626" : "#d1d5db"}
												/>
											))}
										</Scatter>
									</ScatterChart>
				...
				<TabsContent value="comparison" className="space-y-6">
					<Card>
						<CardHeader>
							<CardTitle>
								Performance Heatmap (Top {Math.min(showTop, 15)} Strategies)
							</CardTitle>
						</CardHeader>
						<CardContent>
									...
									<ResponsiveContainer width="100%" height={700}>
										<LineChart
											data={cumulativeReturns.dates.map((date, index) => {
												...
												Object.values(cumulativeReturns.strategies).forEach(
													(strategy) => {
														dataPoint[strategy.name] =
															strategy.cumulative_returns[index] || 0;
													},
												);
												return dataPoint;
											})}
										...
										>
											...
											<ReferenceLine
												...
												strokeDasharray="2 2"
											/>
											{Object.values(cumulativeReturns.strategies).map(
												(strategy, index) => {
													const colors = [
														"#3b82f6",
														"#10b981",
												...
												>
													...
													<div
														className={`text-lg font-bold ${
															strategy.final_return > 0
																? "text-green-600"
																: "text-red-600"
														}`}
													>
														{strategy.final_return > 0 ? "+" : ""}
														{strategy.final_return.toFixed(1)}%
													</div>
						...
						<Card>
							<CardContent className="text-center py-8">
								<p className="text-gray-500">
									Cumulative returns data not available
								</p>
							</CardContent>
						</Card>
					)}
