...
def generate_cumulative_returns(alphas, alpha_results_path):
    ...
    cumulative_data = {"dates": [], "strategies": {}}
    ...
    for alpha in top_alphas:
        if (
            alpha["name"] in alpha_results
            and "returns_data" in alpha_results[alpha["name"]]
        ):
    ...
    for alpha in valid_strategies:
        ...
        while len(cumulative_percentages) < max_days:
            cumulative_percentages.append(
                cumulative_percentages[-1] if cumulative_percentages else 0
            )
    ...
    print(
        f"✅ Using real returns data for {len(valid_strategies)} strategies over {max_days} days"
    )
...
def generate_cumulative_returns_simulated(alphas, num_days=252):
    ...
    for alpha in top_alphas:
        ...
        np.random.seed(
            hash(alpha["name"]) % 2**32
        )  # Consistent seed for reproducibility
        daily_returns = np.random.normal(
            daily_return, volatility / np.sqrt(num_days), num_days
        )
    ...
    print(
        f"✅ Using simulated returns data for {len(top_alphas)} strategies over {num_days} days"
    )
    return cumulative_data
...
def generate_source_code():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "generate_source_code.py"], capture_output=True, text=True
        )
...
def main():
    ...
    if source_code_dest.exists():
        ...
        print("❌ Warning: source_code.json generation may have failed")

    # Generate enhanced metadata
    metadata = {
        "generated_at": "2024-01-01T00:00:00Z",
        "total_strategies": 0,
        "data_sources": ["alpha_leaderboard.json", "source_code.json"],
        "performance_metrics": [
            "sharpe_ratio",
            "annualized_return",
            "calmar_ratio",
