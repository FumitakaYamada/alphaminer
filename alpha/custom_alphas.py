...
class MomentumBreakdownDetection(BaseAlpha):
    ...
    def calculate(self, data: pd.DataFrame, lookback_window: int = 252) -> pd.Series:
        ...
        breakdown_signal[
            ~strong_long & (consistency > (1 - self.consistency_threshold))
        ] = 1.0
...
class VolatilityAdjustedCarry(BaseAlpha):
    ...
    def __init__(
        self, carry_period: int = 60, vol_period: int = 30, target_vol: float = 0.15
    ):
        super().__init__(
...
class InterTemporalArbitrage(BaseAlpha):
    ...
    def __init__(
        self, fast_period: int = 5, slow_period: int = 25, signal_period: int = 10
    ):
        super().__init__(
...
class MicroTrendReversal(BaseAlpha):
    ...
    def __init__(
        self, micro_period: int = 3, trend_period: int = 15, threshold: float = 0.01
    ):
        super().__init__(
    ...
    def calculate(self, data: pd.DataFrame, lookback_window: int = 252) -> pd.Series:
        ...
        medium_trend = (
            medium_trend / (self.trend_period - self.micro_period) * self.micro_period
        )
        ...
        # Reversal conditions
        reversal_signal[strong_micro_down & strong_medium_up] = (
            1.0  # Buy on micro dip in uptrend
        )
        reversal_signal[
            strong_micro_up & strong_medium_down
        ] = -1.0  # Sell on micro rally in downtrend
...
class VolatilitySignalFiltering(BaseAlpha):
    ...
    def __init__(
        self, signal_period: int = 20, vol_period: int = 60, vol_threshold: float = 0.02
    ):
        super().__init__(
